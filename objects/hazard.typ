...
#let mk_hazard(hazard, theme: THEME, short: auto, breakable: auto, hide: ()) = itembox(
    ...
    #if exists(hazard.resistances) {line.push[*Resistances* #hazard.resistances.keys().map(res => [#res #convert_data(hazard.resistances.at(res))]).join(", ")]}
    #if exists(hazard.weaknesses) {line.push[*Weaknesses* #hazard.weaknesses.keys().map(weak => [#weak #convert_data(hazard.weaknesses.at(weak))]).join(", ")]}
    ...
    #let bloc = (
      if exists(hazard.action) and "action" not in hide {
        if type(hazard.action) == dictionary and "class" in hazard.action {
          if hazard.action.class == class_activation {mk_activation(hazard.action, short: short)}
          else if hazard.action.class == class_ability {mk_ability(hazard.action, short: short)}
        }
        else {hazard.action}
      },
    ...
    #let bloc = hazard.activations.map(act => {
      if type(act) == dictionary and "class" in act {
        if act.class == class_activation {mk_activation(act, short: short)}
        else if act.class == class_ability {mk_ability(act, short: short)}
      }
      else {act}
    })
    #blocs.push(bloc.filter(it => exists(it)).join())
